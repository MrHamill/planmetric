...
const submissionId = "22e72512-88de-4ca3-940c-dbaac08b74dd";
...
async function main() {
  console.log("Sending plan email to peterxhamill3@gmail.com...");
  await transporter.sendMail({
    from,
    to: "peterxhamill3@gmail.com",
    subject: `Your ${event} ${level} Plan is Ready — Plan Metric`,
    html: buildPlanEmail("Pete", event, level, planUrl),
  });
  console.log("Sent to peterxhamill3@gmail.com");

  console.log("Sending copy to pete@planmetric.com.au...");
  await transporter.sendMail({
    from,
    to: "pete@planmetric.com.au",
    subject: `Starter Plan Sent: Pete Hamill — ${event} ${level}`,
    html: buildPlanEmail("Pete Hamill", event, level, planUrl),
  });
